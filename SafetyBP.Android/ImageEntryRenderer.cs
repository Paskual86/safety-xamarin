...
namespace SafetyBP.Droid
{
    public class ImageEntryRenderer : EntryRenderer
    {
        ...
        {
            if (imageEntryImage.Trim().Length != 0)
            {
                int resID = Resources.GetIdentifier(System.IO.Path.GetFileNameWithoutExtension(imageEntryImage).ToLower(), nameof(Resource.Drawable).ToLower(), this.Context.PackageName);
